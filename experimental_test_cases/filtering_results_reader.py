...
if __name__ == '__main__':
    ...
    # =====================================================================
    plots_dir = os.path.abspath(os.path.dirname(out_dir_tree_structure_file))
    ...
    if log_scale:
        ...
        plt.semilogy(analysis_times, analysis_rmse, '--', color='darkgreen', linewidth=line_width, markersize=marker_size-0.5, label=filter_name)
        #

    else:
        plt.plot(forecast_times, forecast_rmse, '-d', color='maroon', linewidth=line_width, markersize=marker_size, label='Forecast')
        plt.plot(analysis_times, analysis_rmse, '--', color='darkgreen', linewidth=line_width, markersize=marker_size-0.5, label=filter_name)
    ...
    if log_scale:
        ...
        plty.label('RMSE', fontsize=font_size, fontweight='bold')
    ...
    if mardiaTest_results is not None:
        ...
        #
        file_name = os.path.join(plots_dir, 'Mardia_1.pdf')
        plt.savefig(file_name, dpi=500, facecolor='w', format='pdf', transparent=True, bbox_inches='tight')
        ...
        #
        file_name = os.path.join(plots_dir, 'Mardia_2.pdf')
        plt.savefig(file_name, dpi=500, facecolor='w', format='pdf', transparent=True, bbox_inches='tight')
    ...
    if gmm_results is not None:
        ...
        #
        file_name = os.path.join(plots_dir, 'GMM_1.pdf')
        plt.savefig(file_name, dpi=500, facecolor='w', format='pdf', transparent=True, bbox_inches='tight')
        ...
        if mardiaTest_results is not None:
            ...
            #
            #
            file_name = os.path.join(plots_dir, 'Mardia_3.pdf')
            plt.savefig(file_name, dpi=500, facecolor='w', format='pdf', transparent=True, bbox_inches='tight')
    ...
    if model_name in ['lorenz96', 'coupled lorenz']:
        ignore_indexes = None
    ...
    #
    #
    file_name = os.path.join(plots_dir, 'Forecast_Rhist.pdf')
    #
    ...
    #
    file_name = os.path.join(plots_dir, 'Analysis_Rhist.pdf')
    #
