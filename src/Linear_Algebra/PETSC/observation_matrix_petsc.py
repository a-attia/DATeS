...
from observation_vector_petsc import ObservationVector_PETSCSeq
...
class ObservationMatrixPETScDense(ObservationMatrixBase):
    """
    Initial PETSC implementation of model observation matrix operations.
    ...
    def __init__(self, model_obs_matrix_ref):
        ...
        if model_obs_matrix_ref.shape[0] != model_obs_matrix_ref.shape[1]:
            raise AssertionError("model_obs_matrix_ref passed is not a SQUARE two-dimensional NumpyArray.")

        self._raw_matrix = PETSc.Mat().createDense(model_obs_matrix_ref.shape[-1], array=model_obs_matrix_ref)
    ...
    def vector_product(self, vector, in_place=True):
        ...
        """
        assert isinstance(vector, ObservationVector_PETSCSeq)
        #
    ...
    def solve(self, b):
        ...
        Args:
            b: a ObservationVector_PETSCSeq making up the RHS of the linear system.

        Returns:
            a ObservationVector_PETSCSeq containing the solution to the linear system.
    ...
    def diagonal(self, k=0):
        ...
        if k == 0:
            return ObservationVector_PETSCSeq(self._raw_matrix.getDiagonal().array)
        elif k > 0:
        ...
        #        
        return ObservationVector_PETSCSeq(np_out)
        #
