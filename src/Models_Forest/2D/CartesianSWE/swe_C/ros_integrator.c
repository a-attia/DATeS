...
/* Time                 : Wed Feb 20 11:01:34 2013                  */
/* Working directory    : /home/rossg/kpp-2.2.3/myexamples/cbm4     */
...
{   
   double Ynew[NVAR], Fcn0[NVAR], Fcn[NVAR],
      dFdT[NVAR],
      Jac0[NSPARSE], *Ghimj;//[NVAR*NVAR];
   double K[NVAR*ros_S];   
   double H, T, Hnew, HC, HG, Fac, Tau; 
   double Err, Yerr[NVAR];
   int Pivot[NVAR], Direction, ioffset, j, istage;
   char RejectLastH, RejectMoreH;

   Ghimj = malloc(NVAR*NVAR*sizeof(double));
/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
      ...
      while (1) { /* WHILE STEP NOT ACCEPTED */
        ...
        /*~~~>   Compute the stages  */
         for (istage = 1; istage <= ros_S; istage++) {
            ...
            cblas_dcopy(NVAR,Fcn,1,&K[ioffset],1);
            for (j = 1; j <= istage-1; j++) {
               HC = ros_C[(istage-1)*(istage-2)/2+j-1]/(Direction*H);
               cblas_daxpy(NVAR,HC,&K[NVAR*(j-1)],1,&K[ioffset],1);
            } /* for j */
         
            if ((!Autonomous) && (ros_Gamma[istage-1])) { 
               HG = Direction*H*ros_Gamma[istage-1];
               cblas_daxpy(NVAR,HG,dFdT,1,&K[ioffset],1);
            } /* end if !Autonomous */
            
            SolveTemplate(NVAR, Ghimj, Pivot, &K[ioffset]);
        ...
        /*~~~>  Compute the new solution   */
         cblas_dcopy(NVAR,Y,1,Ynew,1);
         for (j=1; j<=ros_S; j++)
            cblas_daxpy(NVAR,ros_M[j-1],&K[NVAR*(j-1)],1,Ynew,1);

        /*~~~>  Compute the error estimation   */
         cblas_dscal(NVAR,ZERO,Yerr,1);
         for (j=1; j<=ros_S; j++)    
            cblas_daxpy(NVAR,ros_E[j-1],&K[NVAR*(j-1)],1,Yerr,1);
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/   
{
  /*~~~> Local variables */     
   ...
   cblas_daxpy(NVAR,(-ONE),Fcn0,1,dFdT,1);
   cblas_dscal(NVAR,(ONE/Delta),dFdT,1);
...
{   
  /*~~~> Local variables */     
   ...
   swe_jac_full(Jac0, Ghimj);
   
   while (1) {  /* while Singular */
   
  /*~~~>    Construct Ghimj = 1/(H*ham) - Jac0 */
     cblas_dcopy(NSPARSE,Jac0,1,Ghimj,1);
     cblas_dscal(NSPARSE,(-ONE),Ghimj,1);
     ghinv = ONE/(Direction*(*H)*gam);
     for (i=0; i<NVAR; i++) {
       Ghimj[i*NVAR+i] = Ghimj[i*NVAR+i]+ghinv;
     } /* for i */
  /*~~~>    Compute LU decomposition  */
     DecompTemplate( NVAR, Ghimj, Pivot, &ising );
     if (ising == 0) {
  /*~~~>    if successful done  */
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/   
{   
   double gr = (double)1.70710678118655; /* 1.0 + 1.0/SQRT(2.0) */
...
{   
   ...
   *ising = clapack_dgetrf(CblasRowMajor, N, N, A, N, Pivot);
...
{   
  ...
  /*~~~> Note: for a full matrix use Lapack:
   ...
   clapack_dgetrs(CblasRowMajor, CblasNoTrans, N, 1, A, N, Pivot, b, N);
