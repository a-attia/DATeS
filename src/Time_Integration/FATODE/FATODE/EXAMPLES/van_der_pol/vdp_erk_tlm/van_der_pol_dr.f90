...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
   ...
   PROGRAM van_der_pol_dr
    ...
    IMPLICIT NONE
    INTEGER, PARAMETER :: NVAR = 2, NNZ = 0, NTLM = 2
...
!~~>  Control (in) and status (out) arguments for the integration
    ...
    DOUBLE PRECISION :: ATOL(NVAR), RTOL(NVAR), ATOL_TLM(NVAR),RTOL_TLM(NVAR),VAR(NVAR),VAR_TLM(NTLM,NVAR)
...
!~~~> Begin time loop
    ...
    CALL INTEGRATE_TLM(TIN=T,TOUT=TEND,NVAR=NVAR,NTLM=NTLM,NNZ=NNZ,VAR=VAR,RTOL=RTOL,&
          ATOL=ATOL,FUN=FUN,JAC=JAC,RSTATUS_U=RSTATUS,ISTATUS_U=ISTATUS,&
        ICNTRL_U=ICNTRL,RTOL_TLM=RTOL_TLM,ATOL_TLM=ATOL_TLM,VAR_TLM=VAR_TLM)
