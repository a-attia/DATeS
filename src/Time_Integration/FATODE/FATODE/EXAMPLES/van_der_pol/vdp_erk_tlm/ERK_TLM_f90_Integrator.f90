...
CONTAINS
SUBROUTINE INTEGRATE_TLM(TIN,TOUT,NVAR,NTLM,NNZ,VAR,VAR_TLM,RTOL_TLM,ATOL_TLM,&
               RTOL,ATOL,FUN,JAC,ICNTRL_U,RCNTRL_U,ISTATUS_U,RSTATUS_U,IERR_U )

   IMPLICIT NONE
!~~~>  Statistics on the work performed by the ERK method
   INTEGER, INTENT(IN) :: NVAR,NTLM,NNZ
   DOUBLE PRECISION, INTENT(INOUT) :: VAR(NVAR),VAR_TLM(NTLM,NVAR)
   DOUBLE PRECISION, INTENT(IN) :: RTOL(NVAR),RTOL_TLM(NVAR)
   DOUBLE PRECISION, INTENT(IN) :: ATOL(NVAR),ATOL_TLM(NVAR)
  ...
  ! If optional parameters are given, and if they are >0, 
   ...
   T1 = TIN; T2 = TOUT
   CALL ERKTLM(NVAR,NNZ,NTLM,T1,T2,VAR,VAR_TLM,RTOL,ATOL,RTOL_TLM,ATOL_TLM,&
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
   SUBROUTINE ERKTLM(NVAR, NNZ, NTLM, Tinitial, Tfinal, Y, Y_TLM, RelTol, AbsTol, &
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      IMPLICIT NONE 
      INTEGER, INTENT(IN)          :: NVAR, NNZ, NTLM, ICNTRL(20)
      DOUBLE PRECISION, INTENT(IN)    :: Tinitial, Tfinal, &
                    RelTol(NVAR), AbsTol(NVAR), RCNTRL(20), &
                    RelTol_TLM(NVAR,NTLM), AbsTol_TLM(NVAR,NTLM)
      DOUBLE PRECISION, INTENT(INOUT) :: Y(NVAR), Y_TLM(NTLM,NVAR)
...
!~~~> Local variables:      
      DOUBLE PRECISION :: FJAC(NVAR,NVAR),K(NVAR,rkS), TMP(NVAR), SCAL(NVAR), &
                       Yerr_TLM(NVAR,NTLM), &
...
stages:DO istage = 1, rkS
       ...
       IF(istage > 1) THEN
           DO j = 1, istage-1
               CALL DAXPY(NVAR,rkA(istage,j),K(1,j),1,TMP,1)
           END DO
       END IF
 ...
 TlmL: DO itlm = 1,NTLM
          TMP(1:NVAR) = Y_TLM(1:NVAR, itlm)
          IF(istage > 1) THEN
            DO j = 1,istage-1
              CALL DAXPY(NVAR,rkA(istage,j),K_TLM(1,j,itlm),1,TMP,1)
            END DO
          END IF
          K_TLM(:,istage,itlm) = MATMUL(FJAC,TMP)
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      SUBROUTINE ERK_ErrorNorm_tlm(NVAR, NTLM, Y_tlm, FWD_Err)
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
!
      INTEGER :: itlm, NTLM, NVAR
      DOUBLE PRECISION :: Y_tlm(NVAR,NTLM), SCAL_tlm(NVAR), FWD_Err, Err

      DO itlm=1,NTLM
        CALL ERK_ErrorScale(NVAR,ITOL,AbsTol_tlm(1,itlm),RelTol_tlm(1,itlm), &
               Y_tlm(1,itlm),SCAL_tlm)
        CALL ERK_ErrorNorm(NVAR, Y_tlm(1,itlm), SCAL_tlm, Err)
