...
ADJ_MODULE = 'erk_adj_f90_integrator'
ICDIRS = ['/usr/local/include/', '/usr/local/bin/']
SRCFILES = ['ERK_ADJ_f90_Integrator.F90']
...
def create_wrapper(verbose=False):
    ...
    if os.path.isfile(os.path.join(src_dir, extension_module_filename)):
        ...
        try:
            exec("from %s import %s" % (wrapper_signature_suffix, ADJ_MODULE))
            exec("del"+ ADJ_MODULE)
    ...
    if create_module:
        if not os.path.isfile(os.path.join(src_dir, wrapper_signature_filename)):
            ...
            if not os.path.isfile(os.path.join(src_dir, extension_module_filename)):
                print("Failed to wrap the ADJ integrator! For info Check the file %s" %os.path.isfile(os.path.join(src_dir, tmp_outfile)))
                raise IOError
            else:
                if verbose:
