...
!      iterations when used to solve the discrete adjoint)   
   ...
   !ICNTRL(7) = 0    ! Adjoint solution by: 1=Newton, 0=direct
...
!        ICNTRL(7)=1 : modified Newton re-using LU
!        ICNTRL(7)=0 : direct solution (additional one LU factorization per stage)(the default)
!
...
!        Note: if ICNTRL(7)=0 the LU factorization is *not* saved
!
!~~~>  Real parameters
!
...
!~~~> Local variables:      
      ...
      INTEGER :: j, IER, istage, NewtonIter, LastLU
...
!~~~>  Compute E = 1/(h*gamma)-Jac and its LU decomposition
      IF (.NOT.SaveLU .AND. .NOT. DirectADJ) THEN
