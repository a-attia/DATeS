...
!~~~> fine-tune the integrator:
    ...
    ICNTRL(2) = 1       ! 1 = scalar, 2 = vector tolerances
...
!
!    ICNTRL(2)   = 2: AbsTol, RelTol are N-dimensional vectors
...
!~~~>  For Scalar tolerances (ICNTRL(2) == 1)  the code uses AbsTol(1) and RelTol(1)
!   For Vector tolerances (ICNTRL(2) == 2) the code uses AbsTol(1:N) and RelTol(1:N)
    IF (ICNTRL(2) == 2) THEN
      VectorTol = .TRUE.
      UplimTol  = N
    ELSE
      IF (ICNTRL(2) == 1) THEN
        ...
        UplimTol  = 1
      ELSE
        PRINT * , 'User-selected Scalar or Vector tolerances: ICNTRL(2)=',ICNTRL(2)
        CALL ros_ErrorMsg(-10,Tstart,ZERO,IERR)
        RETURN
      END IF
    END IF
...
!  Error indicator
    ...
    PRINT*,'Step=',ISTATUS(Nstp),' Acc=',ISTATUS(Nacc),   &
        ' Rej=',ISTATUS(Nrej), ' Singular=',ISTATUS(Nsng),' Fun=',ISTATUS(Nfun),' Jac=',&
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~   
      ...
      SELECT CASE (Code)
        ...
        CASE (-10)
          PRINT * , '--> Options available: Scalar tolerances [1] or  Vector tolerances [2] '
        CASE DEFAULT
...
! ~~~~ Local variables 
      ...
      DOUBLE PRECISION :: H, HC, HA, Tau, Alpha, Beta, GammaW
      INTEGER :: Direction
...
!~~~> Compute U 
          ...
          DO m = 1,NADJ
            ...
            IF(GetQuad) CALL DAXPY(N,H*ros_W(istage),RY0(1,m),1,U(istart,m),1)
          END DO ! m=1:NADJ
...
!       ~~~~> jac0 jac1 exchanged
          ...
          DO m = 1,NADJ
            ...
            IF(GetQuad)CALL DAXPY(N,H*ros_W(istage),RY1(1,m),1,V(istart,m),1)
          END DO ! m=1:NADJ
...
!           CALL HessTR_Vec ( NHESS, Hes0, U(istart,m), K(istart), Tmp )
            ...
            ! Add (HP0xK_i)^T * U_i
            IF(GetQuad) THEN
              CALL HESSTR_VEC_R(m,N,T,Ystage(1),H*ros_W(istage),K(istart),Tmp)
              CALL DAXPY(N,ONE,Tmp,1,Lambda(1,m),1)
            END IF
          END DO ! m=1:NADJ
        END DO
...
!~~~>  Compute Mu
        ...
        DO istage = 1,ros_S
          istart = N*(istage-1) + 1
         ! Calculate v_bar(i) = (dfdp)^T * u(i) + h * ros_W(i) * (drdp)^T 
          Tau = T + ros_Alpha(istage)*Direction*H
         ...
         ! drdp(T_i,Y_i) of dimension NP x NADJ
          ...
          DO m = 1,NADJ
           ...
           ! Add dfdp^T u_i+drdp^T w_i
            ...
            IF(GetQuad) THEN
              CALL DAXPY(NP,H*ros_W(istage),RP1(1,m),1,Tmp3,1)
              ! Add v_bar (Tmp3)
            ...
            IF(GetQuad) THEN
             ! Add drdpdy term
              CALL HESSTR_VEC_R_PY(m,N,NP,T,Ystage(1),H*ros_W(istage),K(istart),Tmp3)
              CALL DAXPY(NP,ONE,Tmp3,1,Mu(1,m),1)
            END IF
          END DO ! m=1:NADJ
        END DO
...
!~~~>  Types of Adjoints Implemented
   INTEGER, PARAMETER :: Adj_none = 1, Adj_discrete = 2,      &
                   Adj_continuous = 3, Adj_simple_continuous = 4
!~~~>  Checkpoints in memory
   INTEGER, PARAMETER :: bufsize = 2000
   INTEGER :: stack_ptr = 0 ! last written entry
   DOUBLE PRECISION, DIMENSION(:),   POINTER :: chk_H, chk_T
   DOUBLE PRECISION, DIMENSION(:,:), POINTER :: chk_Y, chk_K
...
! ~~~~ Local variables        
      ...
      DOUBLE PRECISION :: H, HC, HA, Tau, Alpha, GammaW 
      INTEGER :: Direction
      INTEGER :: j, m, istage, istart, jstart
!~~~>  Local parameters
      DOUBLE PRECISION, PARAMETER :: ZERO = 0.0d0, ONE  = 1.0d0 
