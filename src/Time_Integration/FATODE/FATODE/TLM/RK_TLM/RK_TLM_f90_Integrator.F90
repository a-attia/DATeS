...
!               Y(1:N) w.r.t. the j-th parameter, j=1...NTLM
    ...
    ICNTRL(7)  = 0   ! TLM solution: 0=iterations, 1=direct
    ICNTRL(9)  = 0   ! TLM Newton iteration error: 0=fwd Newton, 1=TLM Newton error est
    ICNTRL(10) = 1   ! FWD error estimation: 2=classic, 1=SDIRK
...
!		ICNTRL(10) = 2: one additional stage: at c=0, 
!				see Hairer 
!		ICNTRL(10) = 1: two additional stages: one at c=0 
!				and one SDIRK at c=1, stiffly accurate (default)
!
...
!~~~> Error control selection 
      ...
      ELSE
         IF (ICNTRL(10) == 1) THEN
            SdirkError = .TRUE.
         ELSE
            WRITE(6,*) 'ICNTRL(10)=',ICNTRL(10)
            CALL RK_ErrorMsg(-14,T,ZERO,IERR)
         END IF
      END IF
...
!      INTEGER,       DIMENSION(3*N)         :: IPbig
      ...
      DOUBLE PRECISION  :: CONT(N,3), Tdirection,  H, Hacc, Hnew, Hold, Fac, &
		 ...
		 ThetaTLM, ThetaSD
      INTEGER :: i,j,NewtonIter, ISING, Nconsecutive, itlm, LastLU
      INTEGER :: saveNiter, NewtonIterTLM, inf
      LOGICAL :: Reject, FirstStep, SkipJac, NewtonDone, SkipLU,Transp      
       ! maximum times of skipping LU continuously
      INTEGER, PARAMETER :: SLUmax = 20
...
accept:IF (Err < ONE) THEN !~~~> STEP IS ACCEPTED
         ...
         ELSE
            ...
            SkipLU = ( (Theta <= ThetaMin) .AND. (Hratio >= Qmin) &
                 .AND. (Hratio <= Qmax) .AND. (ISTATUS(Nacc)-LastLU) <= SLUmax)
            ! print *,'Theta=',theta,'Hratio=',Hratio,'SkipLu=',SkipLU
            IF (.NOT.SkipLU) THEN
                H = Hnew
                LastLU = ISTATUS(Nacc)
            END IF
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
   ...
   SELECT CASE (Code)
    ...
    CASE (-14)
      PRINT * , '--> Improper values for error control settings'
    CASE DEFAULT
