MODULE ERK_C_FORTRAN_HEADER
  EXTERNAL C_ODE_RHS_InList
  EXTERNAL C_LSS_Norm
  EXTERNAL C_LSS_Norm_InList
  EXTERNAL C_Set2Zero
  EXTERNAL C_Set2Zero_InList
  EXTERNAL C_LSS_DAXPY
  EXTERNAL C_LSS_DAXPY_InList
END MODULE ERK_C_FORTRAN_HEADER
...
CONTAINS
!~~~>  Changes to make it work with DATES
!SUBROUTINE INTEGRATE( TIN, TOUT, NVAR, VAR, RTOL, ATOL, FUN, &
!  ICNTRL_U, RCNTRL_U, ISTATUS_U, RSTATUS_U, Ierr_U )

SUBROUTINE INTEGRATE(NVAR, TIN, TOUT, Y_initial, Y_final, StgVecK, RTOL, ATOL, MODEL, &
    WorkVec, ICNTRL_U, RCNTRL_U, ISTATUS_U, RSTATUS_U, Ierr_U )
   IMPLICIT NONE
...
! This is the initial state, also the solution at a previous time instant
   TYPE(C_PTR), INTENT(IN) :: Y_initial
! This is the solution at the final time step TOUT
   TYPE(C_PTR), INTENT(IN) :: Y_final

!   DOUBLE PRECISION :: RTOL(NVAR)
!~~~>  Changes to make it work with DATES
!   DOUBLE PRECISION :: ATOL(NVAR)
...
!~~~>  Changes to make it work with DATES
   TYPE(C_PTR) , INTENT(IN) :: StgVecK
   TYPE(C_PTR) , INTENT(IN) :: WorkVec
...
!~~~> Changes to make it work with DATES
   DOUBLE PRECISION :: RelTol(NVAR)
   DOUBLE PRECISION :: AbsTol(NVAR)
...
!~~~> Changes to make it work with DATES
   ...
   PRINT *, "INSIDE FORTRAN Integrator"
...
!~~~>  Changes to make it work with DATES
   !~~~> USE Dopri5 integrator FOR NOW
   ICNTRL(3) = 4 ! FIX AT Dopri5
   ICNTRL(2) = 1 ! Scalar tolerances
...
!               RCNTRL,ICNTRL,RSTATUS,ISTATUS,Ierr )
   CALL ERK( NVAR,T1,T2,Y_initial,Y_final,StgVecK,RelTol,AbsTol,MODEL,  &
               WorkVec, RCNTRL,ICNTRL,RSTATUS,ISTATUS,Ierr )
...
!                       RCNTRL, ICNTRL, RSTATUS, ISTATUS, Ierr)
   SUBROUTINE ERK(NVAR, Tinitial, Tfinal,Y_initial, Y_final, StgVecK, RelTol, AbsTol, MODEL, &
                       WorkVec, RCNTRL, ICNTRL, RSTATUS, ISTATUS, Ierr)
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
!
...
!    CALL ERK_Integrator( NVAR,Tinitial,Tfinal,Y,FUN,Ierr )
    CALL ERK_Integrator( NVAR,Tinitial,Tfinal,Y_initial,Y_final,StgVecK,MODEL,WorkVec,Ierr )
...
!   SUBROUTINE ERK_Integrator( NVAR,Tinitial,Tfinal,Y,FUN,Ierr )
   SUBROUTINE ERK_Integrator( NVAR,Tinitial,Tfinal,Y_initial,Y_final,StgVecK,MODEL,WorkVec,Ierr )
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      IMPLICIT NONE
      INTEGER, INTENT(IN) :: NVAR
...
!~~~>  Changes to make it work with DATES
!~~~> Local variables:
!      DOUBLE PRECISION :: K(NVAR,rkS)
!      DOUBLE PRECISION :: TMP(NVAR)
      TYPE(C_PTR) , INTENT(IN) :: WorkVec

      DOUBLE PRECISION :: SCAL(NVAR), T, H, Hratio, Err, Fac, Hnew,Tdirection
      INTEGER :: j, istage
...
!      CALL ERK_ErrorScale(NVAR, ITOL, AbsTol, RelTol, Y, SCAL)
      CALL ERK_ErrorScale(NVAR, ITOL, AbsTol, RelTol, Y_final, SCAL)
...
stages:DO istage = 1, rkS
      ...
      PRINT *, "Start Stage ", istage

!~~~>  Changes to make it work with DATES
!       CALL Set2zero(NVAR,K(1,istage))
       CALL C_Set2Zero_InList(StgVecK, istage)

!~~~>  Changes to make it work with DATES
!       CALL Set2zero(NVAR,TMP)
       CALL C_Set2Zero(WorkVec)

       IF(istage > 1) THEN
           DO j = 1, istage-1
!~~~>  Changes to make it work with DATES
!               CALL DAXPY(NVAR,H*rkA(istage,j),K(1,j),1,TMP,1)
               CALL C_LSS_DAXPY_InList(H*rkA(istage,j),StgVecK,j,WorkVec)
           END DO
       END IF

!~~~>  Changes to make it work with DATES
!       CALL DAXPY(NVAR,ONE,Y,1,TMP,1)
       CALL C_LSS_DAXPY(ONE,Y_final,WorkVec)

!~~~>  Changes to make it work with DATES
!       CALL FUN(NVAR,T+rkC(istage)*H,TMP,K(1,istage))
       CALL C_ODE_RHS_InList(MODEL,T+rkC(istage)*H,WorkVec,StgVecK,istage)
...
!       CALL Set2zero(NVAR,TMP)
      ...
      DO i = 1,rkS
         IF (rkE(i)/=ZERO) THEN
!~~~>  Changes to make it work with DATES
!            CALL DAXPY(NVAR,H*rkE(i),K(1,i),1,TMP,1)
            CALL C_LSS_DAXPY_InList(H*rkE(i),StgVecK,i,WorkVec)
         END IF
      END DO

!~~~>  Changes to make it work with DATES
!      CALL ERK_ErrorNorm(NVAR, TMP, SCAL, Err)
      CALL ERK_ErrorNorm(NVAR, WorkVec, SCAL, Err)
...
!         print *,'Accepted step. Time= ',T,'; Stepsize= ',H
         ...
         DO i = 1,rkS
            IF (rkB(i)/=ZERO) THEN
!~~~>  Changes to make it work with DATES
!              CALL DAXPY(NVAR,H*rkB(i),K(1,i),1,Y,1)
              CALL C_LSS_DAXPY_InList(H*rkB(i),StgVecK,i,Y_final)
            END IF
         END DO
...
!      END IF
      ...
      IF (ITOL == 0) THEN
        CALL C_LSS_Norm(Y, NRM2)
        DR = ( NVAR * AbsTol(1) + RelTol(1) * NRM2 )
        SCAL = ONE / DR
      END IF
...
!
      INTEGER :: i, NVAR
      DOUBLE PRECISION :: SCAL(NVAR), Err
